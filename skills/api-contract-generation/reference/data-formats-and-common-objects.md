...
## x-extensible-enum [#112]

**SHOULD** use `x-extensible-enum` instead of `enum` for values that may grow
over time. This signals to clients that they must handle unknown values
gracefully (tolerant reader pattern [#108]):

```yaml
# Extensible — new values can be added without breaking clients
OrderStatus:
  type: string
  x-extensible-enum:
    - ORDER_PLACED
