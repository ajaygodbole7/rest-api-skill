...
## [#112] SHOULD use open-ended list of values (x-extensible-enum) for enumerations

For enumerations that may grow over time, SHOULD use the `x-extensible-enum`
extension (or the `examples` keyword) instead of a closed `enum`:
...
**Key contract:** When an enum is open (`x-extensible-enum`), consumers MUST handle
unknown values without failing ([#108]). Providers MAY add new values at any time
